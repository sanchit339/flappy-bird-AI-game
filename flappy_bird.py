...
def draw_window(win , birds , pipes , base , score , gen): #the main drawing function
...
def main(genomes , config):
    ...
    for _ , g in genomes: #its genome feature (number:ID and object)
    ...
    while run:
        ...
        for x , bird in enumerate(birds):
            ...
            output = nets[x].activate((bird.y , abs(bird.y - pipes[pipe_ind].height) , abs(bird.y - pipes[pipe_ind].bottom))) #inputs given to the network 
            
            if output[0] > 0.5: #check the output neuron
                bird.jump()
        ...
        for x , bird in enumerate(birds):
            if bird.y + bird.img.get_height() >= 730 or bird.y < 0: #corner check 
                birds.pop(x)
                nets.pop(x)
                ge.pop(x)
...
def run(config_path):
    ...
    winner = p.run(main , 50) #we will use main function as fitness function 
